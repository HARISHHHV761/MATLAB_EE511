...
for Iteration_Var2=2:1:60
    count =0;
    for Iteration_Var3=1:1:Sample_Size
 % This is used to generate X2 to X60 ( If p1<= u <p1+p2 then X=X2)
    if(((Probability)/(Iteration_Var2))<=Random_Number(Iteration_Var3,1)<(((Probability)/(Iteration_Var2-1))+((Probability)/(Iteration_Var2))))
    count=count+1; % This is used to store all the values(Sample_Sizes)which is under Xk
    end
    end
    Vector_A(:,Iteration_Var2)=count; % Store the value of Xk to the Vector_A(i=k)
end
